...
async function doWork(routerId: string, userId: string) {
  ...
  if (!profile?.tenant_id) { await log("error", "Unable to resolve tenant", false); return; }
  const tenantId = profile.tenant_id;

  const { data: router } = await db.from("routers").select("*").eq("id", routerId).eq("tenant_id", tenantId).maybeSingle();
  if (!router) { await log("error", "Router not found", false); return; }
