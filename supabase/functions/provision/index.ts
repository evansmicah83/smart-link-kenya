...
const apiUsername = (router.api_username && router.api_username !== "admin") ? router.api_username : "sln-api";
  ...
  lines.push(
    ...
    safe("/ip pool add name=" + companySlug + "-pool ranges=" + poolStart + "-" + poolEnd),
    "",
    "# 5. DHCP Server",
    safe("/ip dhcp-server remove [find name=" + companySlug + "-dhcp]"),
    safe("/ip dhcp-server add name=" + companySlug + "-dhcp interface=" + bridgeName + " address-pool=" + companySlug + "-pool lease-time=1h disabled=no"),
    safe("/ip dhcp-server network remove [find address=" + subnet + "]"),
    safe("/ip dhcp-server network add address=" + subnet + " gateway=" + gatewayIp + " dns-server=8.8.8.8,8.8.4.4"),
    "",
    "# 6. NAT masquerade on uplink",
    safe('/ip firewall nat remove [find comment="SmartLinkNet NAT"]'),
    safe("/ip firewall nat add chain=srcnat out-interface=" + uplinkInterface + ' action=masquerade comment="SmartLinkNet NAT"'),
    "",
    "# 7. DNS",
    ...
    safe("/radius add service=" + radiusService + " address=" + radiusHost + " secret=" + radiusSecret + " authentication-port=" + radiusAuthPort + " accounting-port=" + radiusAcctPort + ' timeout=3000ms comment="SmartLinkNet"'),
  ...
  if (hasHotspot) {
    ...
    lines.push(
      ...
      safe("/ip hotspot profile add name=" + companySlug + "-hs-profile login-by=http-pap html-directory=hotspot http-cookie-lifetime=1d use-radius=yes accounting=yes login-page=\"" + staticPortalUrl + "\""),
      safe("/ip hotspot add name=" + companySlug + "-hotspot interface=" + bridgeName + " address-pool=" + companySlug + "-pool profile=" + companySlug + "-hs-profile disabled=no"),
      "",
      "# Walled Garden",
      ...
      safe('/ip hotspot walled-garden add dst-host=smart-link-kenya.vercel.app comment="SmartLinkNet portal"'),
      safe('/ip hotspot walled-garden add dst-host=*.supabase.co comment="Supabase"'),
      safe('/ip hotspot walled-garden add dst-host=*.safaricom.com comment="M-Pesa"'),
      safe('/ip hotspot walled-garden add dst-host=mpesa.safaricom.co.ke comment="M-Pesa STK"'),
      safe('/ip hotspot walled-garden ip remove [find comment="HTTPS passthrough"]'),
      safe('/ip hotspot walled-garden ip add dst-address=0.0.0.0/0 protocol=tcp dst-port=443 comment="HTTPS passthrough"'),
    );
  }
  ...
  if (hasPppoe) {
    lines.push(
      ...
      safe('/ppp profile add name=' + companySlug + '-pppoe use-radius=yes comment="SmartLinkNet"'),
      safe("/interface pppoe-server server add name=" + companySlug + "-pppoe interface=" + bridgeName + " default-profile=" + companySlug + "-pppoe disabled=no"),
    );
  }
  ...
  lines.push(
    ...
    safe('/system scheduler add name=sln-poll interval=1m start-time=startup on-event="' + pollOnEvent + '" comment="SmartLinkNet"'),
    "",
    "# 13. Heartbeat scheduler",
    safe("/system scheduler remove [find name=sln-heartbeat]"),
    safe('/system scheduler add name=sln-heartbeat interval=5m start-time=startup on-event="' + heartbeatOnEvent + '" comment="SmartLinkNet"'),
    "",
